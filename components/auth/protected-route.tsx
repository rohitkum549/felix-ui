...
export function ProtectedRoute({ children, requiredRoles = [], requiredResourceRoles = [] }: ProtectedRouteProps) {
  ...
  useEffect(() => {
    ...
    // Set a timeout for authentication check (10 seconds)
    ...
    const checkAuth = () => {
      const token = localStorage.getItem("felix_access_token")
    ...
    // Only check auth if not loading and not already checked
  ...
  console.log("🛡️ ProtectedRoute - isAuthenticated:", isAuthenticated, "isLoading:", isLoading, "hasCheckedAuth:", hasCheckedAuth)
